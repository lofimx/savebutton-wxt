...
3. **Optional desktop app** (`savebutton-gtk` on Linux/macOS, `savebutton-wpf` on Windows) listens on `localhost:21420` and receives copies of files from the extension for users who want disk-level access to `~/.kaya/`. If no desktop app is running, the extension works identically -- it just doesn't mirror files to disk.
...
- The desktop app integration is optional, simplifying the user experience for most people.
