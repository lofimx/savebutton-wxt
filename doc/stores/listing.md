...
Desktop apps for Linux, macOS, and Windows are available for users who want files mirrored to disk at ~/.kaya/. Visit https://savebutton.com for details.
...
- **host_permissions (all URLs)**: Required to sync bookmarks, quotes, and images with the user's self-hosted Save Button server. The server URL is user-configurable, so the extension cannot know the domain in advance. Also used to fetch images from any website when the user saves an image via the context menu, and to communicate with the optional Save Button desktop app on localhost.
